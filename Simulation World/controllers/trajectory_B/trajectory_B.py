...
# Abranda próximo do espelho e da janela
...
class PedestrianFollower(Supervisor):
    def __init__(self):
        ...
        self.filename = "C:/Users/diana/Desktop/psa-pratica/Simulation World/controllers/record_pedestrian/trajeto_B_editado.json"
    ...
    def run(self):
        ...
        while self.step(time_step) != -1:
            ...
            # Definir a posição inicial no primeiro frame
            if self.first_run:
                p = self.waypoints[0]
                self.current_x, self.current_y, self.current_z = p[0], p[1], p[2]
                self.first_run = False
            
            # Ajustar a velocidade conforme a zona
            self.current_speed = self.default_speed
            for (start, end), speed in self.speed_zones.items():
                if start <= self.target_index <= end:
